...
const data = _data.find((item) => item.ids === '1089577016,1089591185');

type LangOverride = Record<string, { title: string; description: string }>;

export default function Credit1089577016() {
  const [lang, setLang] = React.useState<LanguageCode>('zh');

  return (
    <div className="flex flex-col gap-y-4 pb-4 whitespace-pre-line">
      <CreditTitle data={data?.titles as string[]} className="text-3xl!" />
      ...
      {lang && (
        <p className="pt-2 text-2xl font-bold">
          {(data?.langs as unknown as LangOverride)[lang].title}
        </p>
      )}
